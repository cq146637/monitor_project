...
class ActionHandler(object):
    ...
    def action_email(self, action_obj, action_operation_obj, host_id, trigger_data):
        ...
        :return:
        """
        print("------------------------------action email:", action_operation_obj.notifiers, trigger_data)
        ...
        # format 格式化字典
        msg_body = {
            "hostname": models.Host.objects.filter(id=trigger_data.get('host_id')).values('name').first()['name'],
            "ip": models.Host.objects.filter(id=trigger_data.get('host_id')).values('ip_addr').first()['ip_addr'],
            "service_name": trigger_data.get('msg'),
            "time": time.ctime(),
            "msg": trigger_data.get('positive_expressions')
            }

        send_mail(
            subject,
            action_operation_obj.msg_format.format(**msg_body),
                    ...
                    for h in hg.host_set.select_related():
                        ...
                        if h.id == host_id:
                            ...
                            if h.id not in self.alert_counter_dic[action.id]:  # 这个主机第一次触发这个action的报警
                                self.alert_counter_dic[action.id][h.id] = {'counter': 0, 'last_alert': 0}
                ...
                if time.time() - self.alert_counter_dic[action_obj.id][host_id]['last_alert'] >= action_obj.interval:
                    ...
                    for action_operation in action_obj.operations.select_related().order_by('-step'):
                        if action_operation.step > self.alert_counter_dic[action_obj.id][host_id]['counter']:
                            if hasattr(self, 'action_%s' % action_operation.action_type):
