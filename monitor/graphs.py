        ...
        for template in template_list:
            for service in template.services.select_related():
                service_data_dic[service.id] = {
                    ...
                    'index_data': {},  # 有子服务时使用
                    'has_sub_service': service.has_sub_service,  # 是否有子服务
                    'raw_data': [],  # 真实数据列表
                    'items': [item.key for item in service.items.select_related()]  # 有几个指标说明要画几条线
                }
...
class GraphGenerator(object):
    ...
    def get_graph_data(self):
        ...
        if data_set:  # make sure data set not empty
            ...
            if self.sub_service_name is None or self.sub_service_name == 'undefined':
                for data_point in data_set:
                    # data_point sample data:('-->', {
                    # u'status': 0, u'iowait': u'0.00', u'system': u'1.01',
                    # u'idle': u'96.98', u'user': u'2.01', u'steal': u'0.00', u'nice': u'0.00'},
                    # 1461840915.038072)
                                ...
                                for k, v in v_dic.items():
                                    if k in data_dic:
                                        if type(v) is not list:
                                            ...
                                            # 暂时只往前台 返回 average数据
                                            data_dic[k].append([timestamp*1000, float(v[0])])
