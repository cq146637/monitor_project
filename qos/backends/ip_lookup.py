...
class IPLookup(object):
    ...
    def load_db_from_file(self,ip_db_filename):
        db_dic = {'data':{},'keys':[]}
        with open(ip_db_filename, 'rb') as f:
            for line in f:
                ip_range_start,ip_range_end,ip_range_start_int,ip_range_end_int,\
                country,province,city,district,isp,code = line.decode('utf-8').split('|')
                # print(ip_range_end)
